...
function parseDate (datestr) {
  ...
  var tmz = datestr.substring(22, 25)
  var tmzh = tmz / 60
  var tmzm = tmz % 60
...
function wmic (pids, options, done) {
  ...
  var args = [
    ...
    '"' + whereClause + '"',
    'get',
    'CreationDate,KernelModeTime,ProcessId,UserModeTime,WorkingSetSize'
  ]

  bin('wmic', args, {detached: true, windowsHide: true, windowsVerbatimArguments: true}, function (err, stdout) {
    ...
    for (var i = 1; i < stdout.length; i++) {
      ...
      var pid = parseInt(line[2], 10)
      var kerneltime = Math.round(parseInt(line[1], 10) / 10000)
      var usertime = Math.round(parseInt(line[3], 10) / 10000)
      var memory = parseInt(line[4], 10)
    ...
    if (again) {
      return wmic(pids, options, function (err, statistics) {
        if (err) return done(err)
