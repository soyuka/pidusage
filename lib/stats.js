...
var stats = {
  ...
  /**
   * Windows specific function through typeperf
   * @param {int} pid
   * @params {object} options eg: {extra: ['Virtual Bytes', 'Working Set - Private']}
   * @return {function} done (err, stat)
   */
  win: function(pid, options, done) {
    ...
    if (options.extra && Object.prototype.toString.call(options.extra) !== '[object Array]') {
      throw new Error('options.extra must be an array') //we throw because this won't work
    }

    //Launch typeperf command and call callback
    function typePerf(processName) {
      //prepare typeperf command to output something like this: 
      //typeperf -sc 1 "\\COMPUTER-NAME\Process(node)\% Processor Time" "\\COMPUTER-NAME\Process(node)\Working Set"
      var command = ['% Processor Time', 'Working Set']
        .concat(options.extra ? options.extra : [])
        .reduce(function(val, el){
          return val + ' '+ processName + '\\' + el + '"' 
        }, 'typeperf -sc 1')
      ...
      exec(command, function (error, stdout, stderr) {
        ...
        //gather every stat object if any extra options has been provided
    ...
    if (this.history[pid]) { // if we already have in cache the process name as used in typeperf tool
      typePerf(this.history[pid])
    } else {
      //get process prefix
      exec('typeperf "\\Process(*)\\ID Process" -sc 1', function (error, stdout, stderr) {
        ...
        for (var i = 0; i < pidList.length; i++) {
          if (pidList[i].indexOf(p) === 0) {
            self.history[pid] = nameList[i].substr(0, nameList[i].lastIndexOf('\\'))
            break;
          }
        }
        typePerf(self.history[pid])
      })
    }
