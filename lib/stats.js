...
var stats = {
  ...
  /**
   * Windows specific function through windows host scripting
   * @param {int} pid
   * @param {object} options
   * @return {function} done (err, stat)
   */
  win: function(pid, options, done) {
    ...
    exec('cscript /b perf.js '+pid, {cwd: __dirname}, function (error, stdout, stderr) {
      if (error) {
        return done(error)
      }
      var mem, cpu ;
      // do all the calculation in try/catch block to be able to report error
      try {
        var l = stdout.replace(/[\'\"]/g,'').split(os.EOL)
          , values = l[0].split(',');

        cpu = parseFloat(values[1])/os.cpus().length;
        mem =  parseFloat(values[0]);
      }
      catch(e) {
        done(e);
      }
