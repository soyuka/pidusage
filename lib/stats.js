...
var stats = {
	...
	proc: function(pid, options, done) {
		...
		if(this.cpu) {
			this.cpu.uptime = os.uptime()
			this.proc_calc(this.cpu, pid, options, done)
		} else {
			helpers.cpu(function(err, cpu) {
				self.cpu = cpu
				self.proc_calc(cpu, pid, options, done)
			})
		}
	},
	proc_calc: function(cpu, pid, options, done) {
		...
		fs.readFile(p.join('/proc', ''+pid, 'stat'), 'utf8', function(err, infos) {
		    ...
		    var seconds = history.uptime ? cpu.uptime - history.uptime : cpu.uptime - stat.start
