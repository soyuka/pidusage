...
function ps (pid, options, done) {
  ...
  ps.on('close', function (code) {
    ...
    if (code !== 0) return done(new Error('ps exited with code ' + code + ', command was "ps ' + args.join(' ') + '" and error: ' + stderr))
    if (!stdout) {
      return done(null, pids.map(function (pid) {
        return {cpu: 0, memory: 0, pid: parseInt(pid, 10), time: 0, start: new Date(date)}
      }))
    }
